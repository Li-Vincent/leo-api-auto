  ...
  <section class="report">
    ...
    <el-row>
      <el-col :span="24" class="title">
        ...
        <span class="report-id">- ID: {{$route.params.report_id}}</span>
        <span class="execution-info" v-if="testReport.executionUser">执行人：{{testReport.executionUser}}</span>
        <span class="execution-info" v-if="testReport.cronJobId">定时任务：{{testReport.cronJobId}}</span>
      </el-col>
    </el-row>
    ...
    <el-row class="report-basic">
      <el-col :span="10">
        <el-col style="width:160px">
          <span style="text-align:left; font-size: 18px; font-weight: bold">测试基本信息</span>
        </el-col>
        <table style="margin-right:30px;width:70%;height:12px">
          <tbody>
          <tr class="info">
            <td class="label">测试环境</td>
            <td class="text-center">{{testReport.testEnvName}}</td>
          </tr>
          <tr class="info">
            <td class="label">开始时间</td>
            <td class="text-center">{{testReport.testStartTime}}</td>
          </tr>
          <tr class="info">
          ...
          <tr class="info">
            ...
            <td class="text-center">{{testReport.totalCount}}</td>
          </tr>
          <tr class="info">
            <td class="label">通过数</td>
            <td class="text-center">{{testReport.passCount}}</td>
          </tr>
          <tr class="info">
            <td class="label">失败数</td>
            <td class="text-center">{{testReport.failCount}}</td>
          </tr>
          <tr class="info">
            <td class="label">错误数</td>
            <td class="text-center">{{testReport.errorCount}}</td>
          </tr>
          <tr class="info">
            <td class="label">通过率</td>
            <td class="text-center">{{getPassRate(testReport.passCount,testReport.totalCount,true)}}</td>
          </tr>
          </tbody>
        </table>
      </el-col>
...
<script>
    ...
    import {getReportInfo} from "../../../api/testReport";
    ...
    export default {
        ...
        methods: {
            getReport() {
                getReportInfo(this.$route.params.project_id, this.$route.params.report_id).then((res) => {
                        if (res.status === 'ok') {
        ...
        mounted() {
            ...
            this.setChartOptions(this.testReport.passCount, this.testReport.failCount, this.testReport.errorCount);
        },
        watch: {
